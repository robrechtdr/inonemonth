...
def get_representation_for_user(role, user_role):
    ...
    if user_role.type == "juror":
        if role.type == "clencher":
            ...
            if role == user_role:
                return "{0} {1} (me)".format(cap_role_type, role.challenge.get_juror_representation_number(role))
            else:
                return "{0} {1}".format(cap_role_type, role.challenge.get_juror_representation_number(role))
        else:
        ...
        if role.type == "clencher":
            ...
            return "{0} {1} ({2})".format(cap_role_type, role.challenge.get_juror_representation_number(role), role.user.email)
        else:
            raise Exception("Else Die")
