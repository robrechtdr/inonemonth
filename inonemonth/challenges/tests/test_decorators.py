...
from core.tests.setups import (RobrechtClencherRoleFactory,
                               JurorRoleFactory)
from ..decorators import has_github_account


class HasGithubAccountTestCase(django.test.TestCase):
    def test_has_github_account1(self):
        clencher = RobrechtClencherRoleFactory()
        user = clencher.user
        self.assertTrue(has_github_account(user))

    def test_has_github_account2(self):
        # A juror (has no social account but has socialaccount_set attr)
        user = JurorRoleFactory().user
        self.assertFalse(has_github_account(user))

    def test_has_github_account3(self):
