...
def challenge_create_view(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            instance.start_commit = get_last_commit_on_branch(
                github_login,
                instance.repo_name,
                instance.branch_name)
            instance.save()
            return HttpResponseRedirect(
                reverse_lazy("challenge_invite_jurors_view",
...
def invite_jurors_view(request, **kwargs):
    ...
    return render(request=request,
                  template_name='challenge/detail/invite_jurors.html',
...
def challenge_detail_view(request, **kwargs):
    ...
    if request.method == "POST":
        ...
        # http://stackoverflow.com/questions/1395807
        # /proper-way-to-handle-multiple-forms-on-one-page-in-django
        # http://stackoverflow.com/questions/866272
            ...
            if head_comment_form.is_valid() and not challenge.has_ended():
                ...
                if head_comment_id:
                    ...
                    if head_comment.owner == role:
                        ...
                        head_comment.text = head_comment_form \
                            .cleaned_data["text"]
                        head_comment.save()

                        vote = Vote.objects.get(juror=role)
                        vote.decision = head_comment_form \
        ...
        elif "tail-submit" in (head_comment_form.data or
                               tail_comment_form.data):
            if tail_comment_form.is_valid() and not challenge.has_ended():
                ...
                if tail_comment_id:
                    ...
                    if tail_comment.owner == role:
                        ...
                        tail_comment.text = tail_comment_form \
                            .cleaned_data["text"]
                        tail_comment.head = HeadComment.objects.get(
                            id=int(tail_comment_form.data["head-id"]))
                        tail_comment.save()
                    else:
                        return HttpResponseForbidden()

                else:
                    TailComment.objects.create(
                        owner=role,
                        challenge=challenge,
                        text=tail_comment_form.cleaned_data["text"],
                        head=HeadComment.objects.get(
                            id=int(tail_comment_form.data["head-id"])))
    ...
    return render(request=request,
                  template_name='challenge/detail/challenge_detail.html',
