...
def setup_site_and_github_social_app(domain, social_app_name):
    ...
    If github login doesn't work, check if you created a social access
    application on a Github account and if its ID and SECRET are
    identical to the one in settings.
    """
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if args[0] == social_providers[0]:
            ...
            #    "domain '{1}'!".format(
            #        options["social_app_name"],
            #        options["domain"]))
        elif args[0] != social_providers[0]:
            raise CommandError(
                "only the following social providers are "
                "currently supported: {0}".format(", ".join(social_providers)))
